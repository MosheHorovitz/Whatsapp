...
export default function Login({ onReceive }) {
  ...
  const handleMongooseLogin = async (data) => {
    let result = await Api.mongoPopup(data);
    console.log({ result })
    if (result) { onReceive(result) }
  }
  ...
  return (
    <section className="container-login">
      ...
      {loginOptions && <LoginForm loginFunc={handleMongooseLogin} closePopup={handleLoginOption} />}
